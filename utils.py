...
# Use environment MODEL_PATH if available, otherwise use default model name
if 'MODEL_PATH' in os.environ:
    model_name = os.path.join(os.environ['MODEL_PATH'], "BAAI/bge-base-en-v1.5")
else:
    model_name = "BAAI/bge-base-en-v1.5"  # Default model name
...
try:
    ...
    print("Warning: embeddings.joblib not found. You'll need to generate embeddings first.")
    EMBEDDINGS = None
...
def generate_with_single_input(prompt: str, 
                               ...
                               local_server_url: str = "http://127.0.0.1:1234",
                              **kwargs):
    
    # Prepare payload for LM Studio (OpenAI-compatible API)
    payload = {
        "model": model,
    ...
    try:
        ...
        if not response.ok:
            raise Exception(f"Error while calling local LLM: {response.status_code} - {response.text}")
        
        json_dict = response.json()
        
    except requests.exceptions.ConnectionError:
        raise Exception(f"Could not connect to local LLM server at {local_server_url}. Make sure LM Studio is running.")
    except json.JSONDecodeError as e:
        raise Exception(f"Failed to parse JSON response from LLM call.\nException: {e}\nResponse: {response.text}")
    ...
    try:
        ...
        raise Exception(f"Failed to extract output from response. Please try again. Error: {e}")
...
# Load news data if file exists
try:
    NEWS_DATA = pd.read_csv("./news_data_dedup.csv").to_dict(orient = 'records')
except FileNotFoundError:
    print("Warning: news_data_dedup.csv not found. You'll need to provide your news dataset.")
    NEWS_DATA = []


def retrieve(query, top_k = 5):
    if EMBEDDINGS is None:
        raise Exception("Embeddings not loaded. Please generate embeddings first using the model.")
