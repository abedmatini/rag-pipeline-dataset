...
# Mock grading functions to replace dlai_grader dependency
class test_case:
    def __init__(self):
        self.failed = False
        self.msg = ""
...
def print_feedback(cases):
    """Mock function to replace dlai_grader.print_feedback"""
    passed = 0
    total = len(cases)
    ...
    for i, case in enumerate(cases, 1):
        if case.failed:
            ...
            if hasattr(case, 'want') and case.want is not None:
                print(f"   Expected: {case.want}")
            if hasattr(case, 'got') and case.got is not None:
...
try:
    ...
    print("Warning: news_data_dedup.csv not found for testing. Some tests may not work properly.")
    NEWS_DATA = []
